...
# Configure database
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('SQLALCHEMY_DATABASE_URI', 'sqlite:///books.db')
...
def bookshelf():
    """
    Route for the Library page.
    Displays a collection of books that are present in the ReadingProgress table for the current user.
    """
    try:
        user_progress = ReadingProgress.query.filter_by(user_id=current_user.id).all()
        book_ids = [progress.book_id for progress in user_progress]
        books = Book.query.filter(Book.id.in_(book_ids)).all()
        progress_data = {progress.book_id: progress.progress_percentage for progress in user_progress}

        app.logger.debug(f"Fetched {len(books)} books from the ReadingProgress table.")
        app.logger.debug(f"Fetched progress for user {current_user.id}: {progress_data}")

        return render_template('bookshelf.html', books=books, progress_data=progress_data)
    except Exception as e:
        app.logger.error(f"Error fetching library books or user progress: {e}")
        flash('Error fetching library. Please try again later.', 'danger')
        return redirect(url_for('index'))
...
def novels():
    """
    Route for the Novels page.
    Displays a collection of books categorized as novels.
    """
    try:
        novels = Book.query.filter_by(genre='Novel').all()
        return render_template('bookshelf.html', books=novels)
    except Exception as e:
        app.logger.error(f"Error fetching novels: {e}")
        flash('Error fetching novels. Please try again later.', 'danger')
        return redirect(url_for('index'))

@app.route('/shastra_shiksha')
@login_required
def shastra_shiksha():
    """
    Route for the Shastra Shiksha page.
    Displays a collection of books categorized as Shastra Shiksha.
    """
    try:
        shastra_books = Book.query.filter_by(genre='Shastra Shiksha').all()
        return render_template('bookshelf.html', books=shastra_books)
    except Exception as e:
        app.logger.error(f"Error fetching Shastra Shiksha books: {e}")
        flash('Error fetching Shastra Shiksha books. Please try again later.', 'danger')
        return redirect(url_for('index'))

@app.route('/kids_pathshala')
@login_required
def kids_pathshala():
    """
    Route for the Kids Pathshala page.
    Displays a collection of books categorized as Kids Pathshala.
    """
    try:
        kids_books = Book.query.filter_by(genre='Kids Pathshala').all()
        return render_template('bookshelf.html', books=kids_books)
    except Exception as e:
        app.logger.error(f"Error fetching Kids Pathshala books: {e}")
        flash('Error fetching Kids Pathshala books. Please try again later.', 'danger')
        return redirect(url_for('index'))
...
def read_book(book_id):
    ...
    try:
        ...
        last_read_page = progress.last_read_page if progress else 1
